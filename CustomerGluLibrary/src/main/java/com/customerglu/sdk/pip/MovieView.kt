...
class MovieView @JvmOverloads constructor(
    ...
    interface MovieViewInteractionListener {
        fun onMoviewViewClicked()
    }
...
//    private val mToggle: ImageButton
    private lateinit var movieViewInteractionListener: MovieViewInteractionListener
    private lateinit var pipVideoDownloadedListener: PIPVideoDownloadedListener

    private val mShade: View
    var isPaused = false
    lateinit var dataSource: MediaDataSource
...
//    private val mMinimize: ImageButton
    ...
    private var isVideoLooping: Boolean = true
...
//        mMinimize = findViewById(R.id.minimize)
        if (::movieViewInteractionListener.isInitialized) {
            this.setOnClickListener {

                movieViewInteractionListener.onMoviewViewClicked()
            }
        }
        // Attributes
...
//                }
    ...
    fun setMovieViewInteractionListener(movieViewInteractionListener: MovieViewInteractionListener) {
        movieViewInteractionListener.let {
            this.movieViewInteractionListener = it
        }
    }
     ...
     */
    public fun setVideoResourceId(@RawRes id: Int) {
...
//        mMinimize.visibility = View.INVISIBLE
    ...
    fun setCurrentPosition(position: Int) {
        mSavedCurrentPosition = position
    }
    ...
    internal fun openVideo(surface: SurfaceHolder) {
...
//        }
    ...
    fun enableAutoLoopingVideo() {
        mMediaPlayer?.isLooping = true
        isVideoLooping = true
    }

    fun disableAutoLoopingVideo() {
        mMediaPlayer?.isLooping = false
        isVideoLooping = false

    }

    fun isVideoLooping(): Boolean {
        return isVideoLooping
    }
            ...
            try {
                ...
                if (!isPaused) {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        val fileInputStream = FileInputStream(pipHelper.getVideoPath());
...
//        mMediaPlayer?.let {
//            if (it.isPlaying) {
//                mToggle.contentDescription = "pause"
//                mToggle.setImageResource(R.drawable.ic_pause_64dp)
//            } else {
//                mToggle.contentDescription = "play"
